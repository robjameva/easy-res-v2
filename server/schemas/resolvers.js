...
const resolvers = {
	Query: {
		getUser: async (parent, { userId }) => {
			return User.findOne({ _id: userId }).select('-__v');
		},
		getRestaurant: async (parent, { restaurantId }) => {
			...
			const unformattedAvailableHours = operatingHours.filter(
				(item) => !fullHours.includes(item)
			);
		...
		getReservationsByUser: async (parent, { userID }) => {
			return Reservation.find({ user: { _id: userID } }).select('-__v');
		},
	},
	Mutation: {
		...
		createReservation: async (parent, { input }) => {
			...
			const newReservation = await Reservation.find({ _id: reservation._id })
				.populate('user')
				.populate('restaurant');
